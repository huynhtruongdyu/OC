  ...
  if (!config.isProd) {
    ...
    const elapsed =
      meta?.startTime != null
        ...
        : '';
    logger.debug(
      `← ${response.status} ${response.config.method?.toUpperCase()} ${response.config.url}${elapsed}`,
    );
  }
  return response;
};
...
export const onRejected = (
  error: Error & { config?: unknown; response?: unknown },
) => {
  if (!config.isProd) {
    ...
    if (error.response) {
      logger.error(
        `← ${(error.response as { status: number }).status} ${(error.config as { method?: string; url?: string })?.method?.toUpperCase()} ${(error.config as { url?: string })?.url}${elapsed}`,
