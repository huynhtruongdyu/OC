  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Spin spinning={loadingCurrent}>
        {current && <div><Typography.Title level={5}>Current</Typography.Title><WeatherTable data={current} /></div>}
      </Spin>

      <Spin spinning={loadingMock}>
        {mock && <div><Typography.Title level={5}>Mock</Typography.Title><WeatherTable data={mock} /></div>}
      </Spin>

      <Spin spinning={loadingSlow} description="Loading slow (5s delay)...">
        {slow && <div><Typography.Title level={5}>Slow (5s)</Typography.Title><WeatherTable data={slow} /></div>}
      </Spin>

      <Spin spinning={loadingFailed}>
        {failedError && (
          <Alert type="error" showIcon title="Failed Request"
            description={<pre className="whitespace-pre-wrap mb-0">{JSON.stringify(failedError, null, 2)}</pre>} />
        )}
      </Spin>
    </div>
  );
